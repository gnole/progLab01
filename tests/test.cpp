...
TEST(TEST, parser_printer_test) {
  ...
  json j = {
      {"items",
       {{{"name", "Ivanov Petr"},
         ...
         {"avg", "4.25"},
         {"debt", nullptr}},
        {{"name", "Sidorov Ivan"}, {"group", 31}, {"avg", 4}, {"debt", "C++"}},
        {
            {"name", "Pertov Nikita"},
            {"group", "IU8-31"},
            {"avg", 3.33},
  ...
  Student stud_1 = {"Ivanov Petr", std::string("1"), std::string("4.25"),
                    false};
  expectedStudents.push_back(stud_1);
  Student stud_2 = {"Sidorov Ivan", 31, 4, std::string("C++")};
  expectedStudents.push_back(stud_2);
  std::vector<std::string> debt_s3 = {"C++", "Linux", "Network"};
  Student stud_3 = {"Pertov Nikita", std::string("IU8-31"), 3.33, debt_s3};
